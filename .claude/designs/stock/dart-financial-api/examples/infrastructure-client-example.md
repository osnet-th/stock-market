...
```java
// 삼성전자 2023년 사업보고서 재무계정 조회
DartApiResponse<DartSinglAcntItem> response =
    dartApiClient.fetchSingleAccount("00126380", "2023", "11011");

List<DartSinglAcntItem> items = response.getList();

// 다중회사 재무계정 비교 조회 (삼성전자, SK하이닉스)
DartApiResponse<DartMultiAcntItem> multiResponse =
    dartApiClient.fetchMultiAccount("00126380,00164779", "2023", "11011");

// 단일회사 재무지표 - 수익성 지표 조회
DartApiResponse<DartSinglIndxItem> indexResponse =
    dartApiClient.fetchSingleFinancialIndex("00126380", "2023", "11011", "M210000");

// 다중회사 재무지표 - 안정성 지표 비교
DartApiResponse<DartCmpnyIndxItem> multiIndexResponse =
    dartApiClient.fetchMultiFinancialIndex("00126380,00164779", "2023", "11011", "M220000");

// 소송 등의 제기 조회 (기간 검색)
DartApiResponse<DartLawsuitItem> lawsuitResponse =
    dartApiClient.fetchLawsuits("00126380", "20230101", "20231231");
```