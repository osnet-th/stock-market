...
5. **변경 범위 및 승인 원칙**
    - 요청받은 파일과 범위 외의 코드는 수정하지 않습니다.
    - 요청받지 않은 대규모 리팩토링/포맷팅/공개 API 변경을 금지합니다.
    - 연쇄 수정, 구조 변경을 자동으로 확장하지 않습니다.
    - 구조 변경 또는 기존 코드 변경 시 허락을 받은 후 진행합니다.
    - 모든 파일 수정 및 생성은 허락을 받은 후 진행합니다.
    - 한 가지 요청에 한 가지 변경만 수행합니다.
6. **최소 구현 원칙 (YAGNI - You Aren't Gonna Need It)**
    - 명시적으로 요청받지 않은 메서드, 클래스, 기능을 절대 추가하지 않습니다.
    - "나중에 필요할 것 같다"는 이유로 추가 기능을 구현하지 않습니다.
    - 요청된 기능을 구현하는 데 **직접적으로 필요한 코드만** 작성합니다.
7. **메서드 추가 제한**
    - 새로운 public 메서드 추가는 명시적 요청이 있을 때만 가능합니다.
    - private 메서드도 코드 중복 제거 등 명확한 이유 없이 추가하지 않습니다.
    - Helper 메서드, Utility 메서드를 임의로 생성하지 않습니다.
...
10. DI에는 @RequiredArgsConstructor 를 사용합니다.
...
- 구두 설명인 경우, Claude가 설명을 바탕으로 **md 확장자 파일로 초안 작성** (작업 목표, 수정 범위, 대상 파일, 제약 사항 포함)
   ...
   - 작업 계획서를 생성하거나 수정하면 작성일과 수정일을 명시한다.

### Entity 작성

JPA의 Entity 작성 또는 변경이 필요한 경우, 바로 작성하지 않고 **사전 승인 또는 설계 합의 후 진행합니다.**

### 커뮤니케이션

질문을 응답할 때는 항상 태형님이라는 말을 붙여야 합니다.
...
- **테스트는 단위테스트(Unit Test)로 범위를 제한합니다.**
    ...
    - 프레젠테이션 계층(Controller, Config 등)은 TDD를 적용하지 않습니다.
...
- 테스트는 상태를 확인하는 테스트로 작성합니다. verify(), times() 등 호출 횟수를 확인하지 않고, 로직 이후 상태가 원하는 상태인지를 확인합니다.
...
- 반드시 새로운 파일을 추가한 경우 git add 하여 staging area에 추가한다.
- md 파일 작성 시 코드 내용은 작성하지 않는다.
  - 단, 설계 문서는 예외한다.
- 작성일만 명시하고 작성자는 명시하지 않는다.
- 내용 중 데이터베이스 스키마는 작성하지 않는다.
- 작업 리스트 중 완료된 작업은 [완료] 로 변경한다.
- 작업 계획서는 ./claude/plans, 설계는 ./claude/design, 분석은 ./claude/analyzes 에 파일을 저장합니다.
  - 파일 제목은 prefix로 "claude-" 를 반드시 붙입니다.
- 작업 계획서에 내용중 헤더는 번호로 순차를 표시하지 않는다.
- 명시적으로 요청받지 않은 메서드, 클래스, 필드를 추가하지 않는다.
- "확장 가능성", "유연성", "재사용성"을 이유로 요청 범위를 벗어나지 않는다.
- 구현 전에 추가할 메서드/클래스 목록을 명시하고 승인을 받는다.