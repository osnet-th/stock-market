...
## 후속 확장 검증 (저축 지정·순서 변경)
- `./gradlew test` 117/117
- API E2E: 커스텀(구독료) savings=true + 맨 앞 이동 저장 → 응답 순서·savings 반영,
  저축률 0.4574→0.4734(구독료 53,000 산입) / DB sort_order 0..9 재배열·is_savings 확인 /
  커스텀 저축 카테고리 삭제 허용(저축률 복귀) / 기본 저축·투자 삭제 400 유지 /
  시스템 카테고리 savings:true 무시
- 브라우저 E2E: ▲▼로 식비 이동→dirty→저장 후 순서 유지, 커스텀 배지 클릭→'저축' 전환,
  원복 저장까지 왕복 — JS 오류 0건
