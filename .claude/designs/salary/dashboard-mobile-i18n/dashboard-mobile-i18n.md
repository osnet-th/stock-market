...
### Phase 1. 백엔드
- [x] `SpendingCategory` enum에 `displayName` 필드 + 한글 값 추가
- [x] `SpendingLineResponse`에 `categoryLabel` 필드 추가, 팩토리/생성자 반영

### Phase 2. 프론트 (index.html)
- [x] 월급 입력 행을 `sm:` 반응형으로 수정 (라인 3063-3077)
- [x] 카테고리 입력 행을 `sm:` 반응형으로 수정 (라인 3081-3101)
- [x] 월 선택 바 드롭다운 `min-w` 반응형 적용 (월 선택 바는 이미 `flex-wrap` 적용되어 있어 별도 추가 불필요)
- [x] 차트 캔버스 부모 높이 반응형 적용 (라인 3110, 3118, 3131)

### Phase 3. 검증
- [x] `./gradlew compileJava` BUILD SUCCESSFUL — 백엔드 컴파일 검증 완료
- [ ] (수동) dev 서버 기동 후 메인 대시보드 카드의 도넛 범례가 한글로 보이는지 확인
- [ ] (수동) 모바일 뷰포트(예: 375px)에서 입력 폼이 짤리지 않는지 확인
- [ ] (수동) 월 전환/저장/삭제 기존 동작 회귀 없는지 확인
