...
8. 작업 계획서 작성 시 하나의 파일로 전부 작성하지 않고 단계별로 나눠서 작성합니다. 
   - 파일 생성 시 임의로 생성하지 않고 질문 후에 응답 받은 이름으로 생성합니다. 
   - 별도 확장자를 지시하지 않는 경우 작업 계획서의 기본 확장자는 .md 입니다.

### 작업 계획 수립

7. 본격적인 코드 작성 전에, 반드시 **작업 계획을 먼저 작성**합니다.
    - 사용자가 작업 계획서를 제공하거나 구두로 작업 내용을 간단히 설명
    - 구두 설명인 경우, Claude가 설명을 바탕으로 **md 확장자 파일로 초안 작성** (작업 목표, 수정 범위, 대상 파일, 제약 사항 포함)
    - 사용자가 작업 계획서 검토/수정 후 승인
    - 승인 후 Claude가 코드 구현 시작
    - 계획 없이 바로 구현을 시작하지 않습니다.
