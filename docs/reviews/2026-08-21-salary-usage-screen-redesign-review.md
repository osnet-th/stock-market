...
### 후속 확장 리뷰 (저축 지정·순서 변경)
- 순서 UI는 SortableJS 대신 ▲▼ 버튼 — 입력이 밀집한 편집 테이블에서 드래그 delay·revert 패턴
  (favorite.js)의 복잡도 대비 실익이 없다고 판단. 정렬 권위는 서버 sort_order(payload 순서).
- 삭제 보호 완화(커스텀 저축 카테고리는 삭제 가능)는 의도된 동작 — 기본 저축·투자가 항상 남아
  저축률 기준이 유지된다.
- 명시적 findings 없음.
