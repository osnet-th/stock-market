...
- 문제: `glCountNote` 가 4개 섹션만 세어, 섹션 4/4 + 한 줄 정의 빈 용어가 헤더에서는
  '완성'으로 집계되면서 목록 행에는 '한 줄 정의가 비어 있습니다'가 노출되는 모순.
  (목업 `filled()` 로직을 그대로 옮긴 데서 온 것 — 목업 샘플 데이터에는 한 줄 정의가
  항상 채워져 있어 표면화되지 않던 케이스)
- 조치: `glNeedsFill(t)` 추출 — 섹션 미완성 **또는** `oneLine` 공백이면 채움 필요.
  `glCountNote` 가 이를 사용 (`components/glossary.js`).
- 유지: 목록 행 배지의 `N/4 작성` 표기는 목업대로 섹션만 센다 (배지 = 섹션 진행도,
  헤더 카운트 = 미완성 용어 수로 의미가 다름).
- 범위 밖으로 둔 것: 한 줄 정의를 하드 필수로 만들지 않음 (저장 차단 없음) —
  화면 전체가 '저장하고 나중에 채우기' 철학이라 blocking 은 그 철학과 충돌.
- 검증: 목 하네스에 `glNeedsFill` 직접 호출 단언 3건(A8~A10) 추가 — 60항목 PASS.
  기존 픽스처(A5 '채움 필요 3개')는 값 불변 — EBITDA 가 이미 0/4 로 잡혀 있어
  회귀 없음을 함께 확인.

### GitHub Issue 사후 등록

- #118 등록 완료 — docs/issues 문서 status: created 로 갱신.
